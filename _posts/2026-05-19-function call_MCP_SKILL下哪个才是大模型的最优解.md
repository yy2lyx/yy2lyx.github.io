...
- 工作方式：
  ...
  1. 你给模型「函数列表 + 参数定义」
    ```
    使用工具：
    如果遇到你的知识无法回答的问题，使用工具，把使用工具的指令放在<tool>和</tool>中间，使用完工具后你会得到输出放在 <output>和</output>中间
  
    特定使用工具的形式：
    现在你可以使用的工具如下：查询某地、某时问题的函数 Temperature（location, time），使用范例：Temperature（'北京'，'2026-02-22 14:26"）
    ```

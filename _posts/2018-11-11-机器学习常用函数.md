...
#### 1.3 查看数据分布和得到数据报告——pandas_profiling
```python
...
#### 1.5 pandas 抽取Dataframe中一部分行列——loc和iloc区别
...
#### 1.8 计算特征之间的相关系数（自变量之间相关系数较大的话，需要考虑共线性的问题，共线性会导致模型出现开式解，降低模型的稳定性。）
...
#### 1.11  Series.apply()用于对该series的所有元素进行处理生成一个新的series
```python
...
#### 3.9 将label转为one_hot形式——to_categorical
```python
